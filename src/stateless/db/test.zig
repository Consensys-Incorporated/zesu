...
// batch trie update (executor/output.zig) reads that as "no pre-state
// storage" and rebuilds the trie from only the touched slots, silently dropping
// every untouched slot -- a wrong state root reported as success.
//
// db/main.zig's `storage_root_cache.put(...)` sites panic on OOM rather than
// drop the entry or propagate an error (which would be indistinguishable
// upstream from DbError.InvalidWitness), so the invariant
//
//     basic() succeeded  =>  storageRootFor() knows the account's storage root
//
// holds by construction. A panic can't be caught in-process, so it isn't
// exercised here; this test pins the happy path the invariant protects.
test "basic populates the storage-root cache on success" {
    var address: primitives.Address = @splat(0x00);
    address[19] = 0x33;
    const key_hash = mpt.keccak256(&address);
    ...
    var wdb = try db_mod.WitnessDatabase.init(ALLOC, &idx, w.state_root, w.codes, &.{});
    defer wdb.deinit();
...
// whereas `error.InvalidProof` means the witness lacks the node needed to prove
// anything about this slot at all. So InvalidProof means "cannot verify", not
// "absent"; reading it as "slot is zero" would let a wrong state transition be
// executed and proved, so storage() (and the account's storage-root lookup) must
// fail rather than default to zero/empty, matching basic()'s handling of the
// same error.
//
// Here slot 3 genuinely holds 0xabcd, but its storage leaf is withheld from the
// witness, so a zero result would be demonstrably wrong rather than merely
// unproven.
...
// (src/evm/interpreter/host.zig, setupCreateCore): a database error here must
// propagate rather than read as "no storage here" — that would let a CREATE
// succeed at an address the reference rejects, a consensus-level wrong result.
//
// A genuinely absent account is NOT an error: it resolves via valid
// non-inclusion to false, which the second half of this test pins so the
// behavior can't collapse to "always return an error".
